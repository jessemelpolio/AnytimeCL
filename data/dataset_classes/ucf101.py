...
class UCF101(Dataset):
    def __init__(self, root, split='train', transform=None, target_transform=None, seed=0):
        ...
        self.root = os.path.join(root, 'ucf101')
        self.split = split
        ...
        self.categories = sorted(os.listdir(os.path.join(self.root, "UCF-101-midframes")))
        class_to_idx = {c: idx for idx, c in enumerate(self.categories)}
        self.instances = make_dataset(os.path.join(self.root, "UCF-101-midframes"), class_to_idx, IMG_EXTENSIONS)
        cur_dir = os.path.dirname(os.path.realpath(__file__))
        self.classes = json.load(open(os.path.join(cur_dir, "ucf101_classes.json")))
    ...
    def _check_exists(self) -> bool:
        return os.path.exists(os.path.join(self.root, "UCF-101-midframes"))
