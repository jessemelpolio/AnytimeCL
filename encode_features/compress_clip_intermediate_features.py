...
from compression_interface import  pca_compress


def get_clip_compressed_features(model, dataset, store_folder, dataset_type="train"):
    ...
    for bn, batch in enumerate(tqdm(dl)):
        ...
        with torch.no_grad():
            ...
            atten_weight = model.transformer_fix_part[-1].atten_weight.squeeze()
            compressed, principle_components, means, max_p, min_p, max_c, min_c, max_m, min_m = pca_compress(intermediate_feat, args, atten_weight)

            with open(
                os.path.join(cls_folder, f"{num_instances_per_class[cls]}.npz"), "wb"
            ) as f:
                np.savez(f, compressed=compressed.cpu().numpy(),
                            principle=principle_components.cpu().numpy(),
                            means=means.cpu().numpy(),
                        )
                if args.int_quantize:
                    np.savez(f,max_p=max_p.cpu().numpy(),
                               min_p=min_p.cpu().numpy(),
                               max_c=max_c.cpu().numpy(),
                               min_c=min_c.cpu().numpy(),
                               max_m=max_m.cpu().numpy(),
                               min_m=min_m.cpu().numpy()
                            )
